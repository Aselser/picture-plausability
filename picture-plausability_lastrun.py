...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\agust\\OneDrive\\Desktop\\CNC\\Paradigmas\\Python\\Rutvik\\picture-plausability\\picture-plausability_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    practice_instructions_text_1 = visual.TextStim(win=win, name='practice_instructions_text_1',
        text='',
        font='Open Sans',
        ...
        languageStyle='LTR',
        depth=0.0);
    key_resp_practice_instructions_1 = keyboard.Keyboard()
    
    # --- Initialize components for Routine "practice_trials_instructions_2" ---
    practice_instructions_text_2 = visual.TextStim(win=win, name='practice_instructions_text_2',
        text='',
        font='Open Sans',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    key_resp_practice_instructions_2 = keyboard.Keyboard()
    ...
    practice_instructions_text_3 = visual.TextStim(win=win, name='practice_instructions_text_3',
        ...
        languageStyle='LTR',
        depth=0.0);
    key_resp_practice_instructions_3 = keyboard.Keyboard()
    ...
    trials_2 = data.TrialHandler(nReps=1.0, method='sequential', 
        ...
        trialList=data.importConditions('resources/trials.xlsx'),
    ...
    for thisTrial_2 in trials_2:
        ...
        practice_instructions_text_1.setText(open(f'resources/{category}/instructions/instructions1.txt', encoding='utf-8').read())
        key_resp_practice_instructions_1.keys = []
        key_resp_practice_instructions_1.rt = []
        _key_resp_practice_instructions_1_allKeys = []
        # keep track of which components have finished
        practice_trials_instructionsComponents = [practice_instructions_text_1, key_resp_practice_instructions_1]
        ...
        while continueRoutine:
            ...
            # *practice_instructions_text_1* updates
            
            # if practice_instructions_text_1 is starting this frame...
            if practice_instructions_text_1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                practice_instructions_text_1.frameNStart = frameN  # exact frame index
                practice_instructions_text_1.tStart = t  # local t and not account for scr refresh
                practice_instructions_text_1.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(practice_instructions_text_1, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'practice_instructions_text_1.started')
                # update status
                practice_instructions_text_1.status = STARTED
                practice_instructions_text_1.setAutoDraw(True)
            
            # if practice_instructions_text_1 is active this frame...
            if practice_instructions_text_1.status == STARTED:
                # update params
                pass
            
            # *key_resp_practice_instructions_1* updates
            waitOnFlip = False
            
            # if key_resp_practice_instructions_1 is starting this frame...
            if key_resp_practice_instructions_1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                key_resp_practice_instructions_1.frameNStart = frameN  # exact frame index
                key_resp_practice_instructions_1.tStart = t  # local t and not account for scr refresh
                key_resp_practice_instructions_1.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(key_resp_practice_instructions_1, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'key_resp_practice_instructions_1.started')
                # update status
                key_resp_practice_instructions_1.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(key_resp_practice_instructions_1.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(key_resp_practice_instructions_1.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if key_resp_practice_instructions_1.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_practice_instructions_1.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
                _key_resp_practice_instructions_1_allKeys.extend(theseKeys)
                if len(_key_resp_practice_instructions_1_allKeys):
                    key_resp_practice_instructions_1.keys = _key_resp_practice_instructions_1_allKeys[-1].name  # just the last key pressed
                    key_resp_practice_instructions_1.rt = _key_resp_practice_instructions_1_allKeys[-1].rt
                    key_resp_practice_instructions_1.duration = _key_resp_practice_instructions_1_allKeys[-1].duration
        ...
        # check responses
        if key_resp_practice_instructions_1.keys in ['', [], None]:  # No response was made
            key_resp_practice_instructions_1.keys = None
        trials_2.addData('key_resp_practice_instructions_1.keys',key_resp_practice_instructions_1.keys)
        if key_resp_practice_instructions_1.keys != None:  # we had a response
            trials_2.addData('key_resp_practice_instructions_1.rt', key_resp_practice_instructions_1.rt)
            trials_2.addData('key_resp_practice_instructions_1.duration', key_resp_practice_instructions_1.duration)
        ...
        practice_instructions_text_2.setText(open(f'resources/{category}/instructions/instructions2.txt', encoding='utf-8').read())
        key_resp_practice_instructions_2.keys = []
        key_resp_practice_instructions_2.rt = []
        _key_resp_practice_instructions_2_allKeys = []
        # keep track of which components have finished
        practice_trials_instructions_2Components = [practice_instructions_text_2, key_resp_practice_instructions_2]
        ...
        while continueRoutine:
            ...
            # *practice_instructions_text_2* updates
            
            # if practice_instructions_text_2 is starting this frame...
            if practice_instructions_text_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                practice_instructions_text_2.frameNStart = frameN  # exact frame index
                practice_instructions_text_2.tStart = t  # local t and not account for scr refresh
                practice_instructions_text_2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(practice_instructions_text_2, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'practice_instructions_text_2.started')
                # update status
                practice_instructions_text_2.status = STARTED
                practice_instructions_text_2.setAutoDraw(True)
            
            # if practice_instructions_text_2 is active this frame...
            if practice_instructions_text_2.status == STARTED:
                # update params
                pass
            
            # *key_resp_practice_instructions_2* updates
            waitOnFlip = False
            
            # if key_resp_practice_instructions_2 is starting this frame...
            if key_resp_practice_instructions_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                key_resp_practice_instructions_2.frameNStart = frameN  # exact frame index
                key_resp_practice_instructions_2.tStart = t  # local t and not account for scr refresh
                key_resp_practice_instructions_2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(key_resp_practice_instructions_2, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'key_resp_practice_instructions_2.started')
                # update status
                key_resp_practice_instructions_2.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(key_resp_practice_instructions_2.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(key_resp_practice_instructions_2.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if key_resp_practice_instructions_2.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_practice_instructions_2.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
                _key_resp_practice_instructions_2_allKeys.extend(theseKeys)
                if len(_key_resp_practice_instructions_2_allKeys):
                    key_resp_practice_instructions_2.keys = _key_resp_practice_instructions_2_allKeys[-1].name  # just the last key pressed
                    key_resp_practice_instructions_2.rt = _key_resp_practice_instructions_2_allKeys[-1].rt
                    key_resp_practice_instructions_2.duration = _key_resp_practice_instructions_2_allKeys[-1].duration
                    # a response ends the routine
                    continueRoutine = False
        ...
        # check responses
        if key_resp_practice_instructions_2.keys in ['', [], None]:  # No response was made
            key_resp_practice_instructions_2.keys = None
        trials_2.addData('key_resp_practice_instructions_2.keys',key_resp_practice_instructions_2.keys)
        if key_resp_practice_instructions_2.keys != None:  # we had a response
            trials_2.addData('key_resp_practice_instructions_2.rt', key_resp_practice_instructions_2.rt)
            trials_2.addData('key_resp_practice_instructions_2.duration', key_resp_practice_instructions_2.duration)
        ...
        key_resp_practice_instructions_3.keys = []
        key_resp_practice_instructions_3.rt = []
        _key_resp_practice_instructions_3_allKeys = []
        # keep track of which components have finished
        practice_trials_instructions_3Components = [practice_instructions_text_3, key_resp_practice_instructions_3]
        ...
        while continueRoutine:
            ...
            # *key_resp_practice_instructions_3* updates
            waitOnFlip = False
            
            # if key_resp_practice_instructions_3 is starting this frame...
            if key_resp_practice_instructions_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                key_resp_practice_instructions_3.frameNStart = frameN  # exact frame index
                key_resp_practice_instructions_3.tStart = t  # local t and not account for scr refresh
                key_resp_practice_instructions_3.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(key_resp_practice_instructions_3, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'key_resp_practice_instructions_3.started')
                # update status
                key_resp_practice_instructions_3.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(key_resp_practice_instructions_3.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(key_resp_practice_instructions_3.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if key_resp_practice_instructions_3.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_practice_instructions_3.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
                _key_resp_practice_instructions_3_allKeys.extend(theseKeys)
                if len(_key_resp_practice_instructions_3_allKeys):
                    key_resp_practice_instructions_3.keys = _key_resp_practice_instructions_3_allKeys[-1].name  # just the last key pressed
                    key_resp_practice_instructions_3.rt = _key_resp_practice_instructions_3_allKeys[-1].rt
                    key_resp_practice_instructions_3.duration = _key_resp_practice_instructions_3_allKeys[-1].duration
                    # a response ends the routine
                    continueRoutine = False
        ...
        # check responses
        if key_resp_practice_instructions_3.keys in ['', [], None]:  # No response was made
            key_resp_practice_instructions_3.keys = None
        trials_2.addData('key_resp_practice_instructions_3.keys',key_resp_practice_instructions_3.keys)
        if key_resp_practice_instructions_3.keys != None:  # we had a response
            trials_2.addData('key_resp_practice_instructions_3.rt', key_resp_practice_instructions_3.rt)
            trials_2.addData('key_resp_practice_instructions_3.duration', key_resp_practice_instructions_3.duration)
        ...
        trials_practice = data.TrialHandler(nReps=1.0, method='random', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions( f'resources/{category}/stimuli/practice.xlsx'),
        ...
        for thisTrials_practice in trials_practice:
            ...
            practice_image.setImage(file)
            # keep track of which components have finished
            stim_practiceComponents = [key_resp_practice_stim, practice_image]
            ...
            while continueRoutine and routineTimer.getTime() < 4.5:
                ...
                if key_resp_practice_stim.status == STARTED and not waitOnFlip:
                    theseKeys = key_resp_practice_stim.getKeys(keyList=['j','k'], ignoreKeys=["escape"], waitRelease=False)
                    ...
                    if len(_key_resp_practice_stim_allKeys):
                        ...
                        # was this correct?
                        if (key_resp_practice_stim.keys == str(correct)) or (key_resp_practice_stim.keys == correct):
                ...
                # if practice_image is stopping this frame...
                if practice_image.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > practice_image.tStartRefresh + 4.5-frameTolerance:
                        # keep track of stop time/frame for later
                        ...
                        # update status
                        practice_image.status = FINISHED
            ...
            feedback_text = 'CORRECTO' if correct == key_resp_practice_stim.keys else 'INCORRECTO'
        ...
        trials_2.addData('key_resp_instructiones.keys',key_resp_instructiones.keys)
        if key_resp_instructiones.keys != None:  # we had a response
            trials_2.addData('key_resp_instructiones.rt', key_resp_instructiones.rt)
            trials_2.addData('key_resp_instructiones.duration', key_resp_instructiones.duration)
        ...
        trials = data.TrialHandler(nReps=1.0, method='random', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions(f'resources/{category}/stimuli/stimuli.xlsx'),
        ...
        for thisTrial in trials:
            ...
            while continueRoutine and routineTimer.getTime() < 4.5:
                ...
                if image.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > image.tStartRefresh + 4.5-frameTolerance:
                        # keep track of stop time/frame for later
